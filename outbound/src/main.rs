...
fn main() {
    ...
    // flush rules for inbound and outbound traffic
    let _ = Command::new("sudo")
        .args(&["iptables", "-F"])
        .status();

        let configure_iptables_msg = format!("iptables flushed rules with strict policy based on the IP address {} and port {} from the file /var/ossec/etc/ossec.conf.", ip, port);
